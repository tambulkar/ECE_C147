...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            output, cache = affine_relu_forward(
                input, self.params[f"W{i}"], self.params[f"b{i}"]
            )
            input = output
            caches.append(cache)
        ...
        loss += (
            ...
            * sum(
                [
                    np.linalg.norm(self.params[f"W{i}"]) ** 2
                    for i in range(1, self.num_layers + 1)
                ]
            )
        )
        ...
        for i in range(self.num_layers - 1, 0, -1):
            dx, grads[f"W{i}"], grads[f"b{i}"] = affine_relu_backward(dx, caches[i - 1])
