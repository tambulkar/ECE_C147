...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # ================================================================ #
        h1 = np.maximum(np.dot(X, W1.T) + b1, 0)
        ...
        loss = (
            np.sum(np.log(np.sum(exp_scores, axis=1)) - scores[np.arange(N), y]) / N
        ...
        # ================================================================ #
        exp_score_sums = np.sum(exp_scores, axis=1)[:, np.newaxis]
        probs = exp_scores / exp_score_sums
        probs[np.arange(N), y] -= 1

        dL_da = probs / N
        dL_dc2 = dL_da
        dL_db2 = np.sum(dL_da, axis=0)
        dc2_dW2 = h1
        grads["W2"] = np.dot(dL_dc2.T, dc2_dW2) + (reg * W2)
        grads["b2"] = dL_db2

        dc2_dh1 = W2
        dL_dh1 = (h1 > 0) * np.dot(dL_dc2, dc2_dh1)
        dL_db1 = np.sum(dL_dh1, axis=0)
        dh1_dW1 = X
        dL_dW1 = np.dot(dL_dh1.T, dh1_dW1) + reg * W1
        grads["W1"] = dL_dW1
        grads["b1"] = dL_db1
        # ================================================================ #
        # END YOUR CODE HERE
    ...
    def train(
        ...
        for it in np.arange(num_iters):
            ...
            sample_indices = np.random.choice(
                np.arange(num_train), min(num_train, batch_size)
            )
    ...
    def predict(self, X):
        ...
        W1, b1 = self.params["W1"], self.params["b1"]
        W2, b2 = self.params["W2"], self.params["b2"]
        h1 = np.maximum(np.dot(W1, X.T) + b1[:, np.newaxis], 0)
        scores = np.dot(W2, h1) + b2[:, np.newaxis]
        exp_scores = np.exp(scores)
